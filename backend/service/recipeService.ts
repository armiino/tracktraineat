  ...
  async getMealPlan(userId: string, dto: RecipeQueryDto) {
    ...
    for (const [shareKey, indices] of Object.entries(groups)) {
      ...
      for (let i = 0; i < indices.length; i++) {
        ...
        if (group.length > 0) {
          meals[`meal${idx + 1}`] = group.map(r => ({ ...r, source: 'standard' }));
        } else {
          // Fallback-Logik
          ...
          for (let j = 0; j < parts.length; j++) {
            ...
            if (splitRecipes.length === 0) {
              ...
              if (splitRecipes.length > 0) {
                warnings.push(`Mahlzeit ${idx + 1}${fallbackLabels[j]} wurde ohne Proteinziel erstellt.`);
              }
            }
            ...
            if (splitRecipes.length > 0) {
              ...
              warnings.push(`Für Mahlzeit ${idx + 1}${fallbackLabels[j]} konnten keine Rezepte gefunden werden.`);
            }
          }
        }
      }
